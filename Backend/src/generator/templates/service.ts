...
export function renderService(model: GenerationModel, klass: GenClass): string {
  ...
  // Si la PK no la autogenera JPA (String, fechas, etc.), el cliente la manda
  // en el RequestDTO como "id" y hay que asignarla a mano antes de guardar;
  // si no, "create" nunca la setea y persist() explota (IdentifierGenerationException).
  const pkAssignment = isAutoGeneratedPk(klass.pkAttribute.javaType)
    ? ''
    : `        entity.set${capitalize(klass.pkAttribute.fieldName)}(dto.getId());\n`;
...
${autowiredRefs ? autowiredRefs + '\n\n' : ''}    public List<${klass.className}ResponseDTO> findAll() {
    ...
    public ${klass.className}ResponseDTO findById(${klass.pkAttribute.javaType} id) {
        ${klass.className} entity = repository.findById(id)
            .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "${klass.className} no encontrado: " + id));
...
${pkAssignment}        applyDto(entity, dto);
    ...
    public void delete(${klass.pkAttribute.javaType} id) {
        if (!repository.existsById(id)) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "${klass.className} no encontrado: " + id);
        }
